<script lang="ts">
  ...
  type ByHourResponse = { buckets: Bucket[] };
  ...
  async function load() {
    ...
    try {
      ...
      // Fill missing hours with 0 so the chart always shows 0-23.
      const byHour = new Map(data.buckets.map((b) => [b.hour, b.count]));
      const full = Array.from({ length: 24 }, (_, h) => ({ hour: h, count: byHour.get(h) ?? 0 }));
      totalComplaints = full.reduce((s, b) => s + b.count, 0);
      const peak = full.reduce((p, b) => (b.count > p.count ? b : p));
      peakHour = peak.hour;
        ...
        container,
        {
          ...
          encoding: {
            x: {
              ...
              type: "ordinal",
              title: "Hour of day",
              axis: { labelColor: "#4d5b65", titleColor: "#4d5b65", grid: false, labelAngle: 0 }
            },
            y: {
              field: "count",
              type: "quantitative",
            ...
            color: {
              field: "count",
              type: "quantitative",
              scale: { range: ["#fed976", "#fd8d3c", "#fc4e2a", "#b10026"] },
              legend: null
            },
            tooltip: [
...
<article class="chart-card" use:inView={load}>
  <header class="chart-header">
    <div>
      <h3>{title}</h3>
      <p class="chart-sub">distribution over the 24-hour clock</p>
    </div>
    {#if status === "ready"}
      <div class="chart-stats">
        <span><strong>{peakHour}h</strong> peak</span>
  ...
  <div class="chart-body">
    {#if status === "loading"}
      <div class="state loading"><div class="spinner"></div>Loading hourly distribution...</div>
    {:else if status === "empty"}
      <div class="state empty">No timestamped complaints found.</div>
